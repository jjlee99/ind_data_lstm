...
def create_sliding_windows(data_series, input_sequence_length, output_sequence_length, stride, target_feature_index=0):
    """
    ...
    Args:
        ...
        target_feature_index (int): 다중 특성 데이터에서 타겟 y로 사용할 특성의 인덱스.

    Returns:
        tuple: (X, y)
            X (np.array): 입력 데이터. 형태: (샘플 수, input_sequence_length, num_features)
            y (np.array): 타겟 데이터. 형태: (샘플 수, output_sequence_length, 1)
    """
    if isinstance(data_series, pd.Series):
        data_values = data_series.values
    else:
        data_values = data_series

    if data_values.ndim == 1:
        data_values = data_values.reshape(-1, 1) # (n_points, 1 feature) 형태로 변환
    
    num_features = data_values.shape[1]

    X_list, y_list = [], []
    total_data_length = len(data_values)
    ...
    for i in range(0, total_data_length - total_sample_length + 1, stride):
        input_seq = data_values[i : i + input_sequence_length] # 모든 특성 포함
        # 타겟은 지정된 특성만 사용
        output_seq = data_values[i + input_sequence_length : i + input_sequence_length + output_sequence_length, target_feature_index:target_feature_index+1]
        X_list.append(input_seq)
        y_list.append(output_seq)

    if not X_list:
        return np.array([]).reshape(0, input_sequence_length, num_features), np.array([]).reshape(0, output_sequence_length, 1)
...
df_for_features = df.copy() # MA, MSTD 추가 및 NaN 제거용 DataFrame

# --- 이동 평균 및 이동 표준편차 특성 추가 ---
POINTS_PER_DAY = 24 * (60 // 5) # 하루에 288 포인트 (5분 간격)
MA_STD_WINDOW_SIZE = POINTS_PER_DAY // 2 # 12시간 윈도우 (144 포인트)

print(f"\n'{TARGET_COLUMN}'에 대해 이동 평균 및 이동 표준편차(윈도우: {MA_STD_WINDOW_SIZE} 포인트) 특성 추가 중...")
df_for_features['MA'] = df_for_features[TARGET_COLUMN].rolling(window=MA_STD_WINDOW_SIZE).mean()
df_for_features['MSTD'] = df_for_features[TARGET_COLUMN].rolling(window=MA_STD_WINDOW_SIZE).std()

initial_len = len(df_for_features)
df_for_features.dropna(inplace=True)
rows_dropped_by_ma_std = initial_len - len(df_for_features)
print(f"MA/MSTD 계산 후 NaN 값 제거: {rows_dropped_by_ma_std} 행이 제거되었습니다.")
if df_for_features.empty:
    raise ValueError("MA/MSTD 특성 추가 및 NaN 제거 후 데이터가 없습니다. 원본 데이터 길이나 윈도우 크기를 확인해주세요.")
print(f"특성 추가 후 데이터프레임 길이: {len(df_for_features)}")

df_processed_features = df_for_features.reset_index(drop=True) # 인덱스 재설정

# --- 스케일링 (TARGET_COLUMN, MA, MSTD 특성에 대해) ---
features_to_scale = df_processed_features[[TARGET_COLUMN, 'MA', 'MSTD']].values
scaler = MinMaxScaler()
series_scaled = scaler.fit_transform(features_to_scale) # (N, 3) 형태

# --- 슬라이딩 윈도우 파라미터 설정 ---
# POINTS_PER_DAY는 위에서 이미 정의됨
...
if len(series_scaled) < (INPUT_SEQ_LENGTH + OUTPUT_SEQ_LENGTH):
    raise ValueError(
        f"장비 '{SELECTED_EQUIP_NO}'의 '{TARGET_COLUMN}'에 대한 특성 데이터(스케일링 전)가 너무 적어 ({len(series_scaled)} 포인트) "
        f"윈도우(입력 {INPUT_SEQ_LENGTH}, 출력 {OUTPUT_SEQ_LENGTH})를 생성할 수 없습니다. "
        "데이터 기간을 확인하거나 윈도우 크기를 줄여주세요."
    )

X, y = create_sliding_windows(series_scaled, # 스케일링된 다중 특성 데이터 사용
                               input_sequence_length=INPUT_SEQ_LENGTH,
                               output_sequence_length=OUTPUT_SEQ_LENGTH,
                               stride=STRIDE_POINTS,
                               target_feature_index=0) # TARGET_COLUMN이 첫 번째 특성

if X.shape[0] == 0:
    raise ValueError(f"생성된 윈도우 샘플이 없습니다 (X shape: {X.shape}). 데이터 양이나 윈도우/stride 설정을 확인하세요.")
...
model = GasDemandLSTM(input_size=3, hidden_size=64, output_sequence_length=OUTPUT_SEQ_LENGTH, num_layers=2).to(device) # input_size=3
...
if not all_val_preds_scaled_list:
    ...
    if all_val_preds_scaled.shape[0] > 0:
        # 첫 번째 검증 샘플의 예측된 TARGET_COLUMN (스케일링 된 상태)
        val_pred_sample_scaled_target = all_val_preds_scaled[0] # (OUTPUT_SEQ_LENGTH, 1)
        # 첫 번째 검증 샘플의 실제 TARGET_COLUMN (스케일링 된 상태)
        val_actual_sample_scaled_target = all_val_actuals_scaled[0] # (OUTPUT_SEQ_LENGTH, 1)

        # 스케일 역변환 (3개 특성으로 확장 후 역변환)
        dummy_pred_for_inverse = np.zeros((OUTPUT_SEQ_LENGTH, 3))
        dummy_pred_for_inverse[:, 0] = val_pred_sample_scaled_target.flatten()
        val_pred_reconstructed = scaler.inverse_transform(dummy_pred_for_inverse)[:, 0]

        # 실제값은 original_target_series_values에서 가져옴
        # 첫 검증 샘플의 타겟 시퀀스가 시작하는 인덱스 (처리된 데이터 기준)
        first_val_target_start_idx_in_processed = train_size * STRIDE_POINTS + INPUT_SEQ_LENGTH
        # 원본 데이터에서의 해당 인덱스
        first_val_target_start_idx_in_original = first_val_target_start_idx_in_processed + rows_dropped_by_ma_std
        
        if first_val_target_start_idx_in_original + OUTPUT_SEQ_LENGTH <= len(original_target_series_values):
            val_actual_reconstructed = original_target_series_values[
                first_val_target_start_idx_in_original : first_val_target_start_idx_in_original + OUTPUT_SEQ_LENGTH
            ]
        else:
            print("경고: 첫 번째 검증 샘플의 실제값을 원본 데이터에서 가져올 수 없습니다 (인덱스 초과).")
            val_actual_reconstructed = np.full_like(val_pred_reconstructed, np.nan) # 임시로 NaN 처리

        fig, ax = plt.subplots(figsize=(14, 7))
        time_steps_val = range(len(val_actual_reconstructed))
        sns.lineplot(x=time_steps_val, y=val_actual_reconstructed, ax=ax, label="Actual (Validation First Sample)")
        sns.lineplot(x=time_steps_val, y=val_pred_reconstructed, ax=ax, label="Predicted (Validation First Sample)")
        ...
        fig.savefig(file_path)
        print(f"검증 데이터 첫 샘플 예측 그래프 저장: {file_path}")
        plt.show()
    else:
...
if len(X_val) > 0 and len(y_val) > 0:
    ...
    # 1. 검증 기간 전체의 실제값(원본 스케일) 플로팅 (df_original_for_plotting 기준)
    #    첫 검증 윈도우의 타겟 시작점부터 마지막 검증 윈도우의 타겟 끝점까지
    val_period_target_start_idx_in_processed = train_size * STRIDE_POINTS + INPUT_SEQ_LENGTH
    val_period_target_end_idx_in_processed = (train_size + len(X_val) - 1) * STRIDE_POINTS + INPUT_SEQ_LENGTH + OUTPUT_SEQ_LENGTH
    
    val_period_target_start_idx_in_original = val_period_target_start_idx_in_processed + rows_dropped_by_ma_std
    val_period_target_end_idx_in_original = val_period_target_end_idx_in_processed + rows_dropped_by_ma_std

    if val_period_target_end_idx_in_original <= len(original_target_series_values):
        actual_val_period_timestamps = df_original_for_plotting['REPORT_DATETIME'].iloc[val_period_target_start_idx_in_original:val_period_target_end_idx_in_original]
        actual_val_period_original_values = original_target_series_values[val_period_target_start_idx_in_original:val_period_target_end_idx_in_original]
        ...
        with torch.no_grad():
            for i_val_sample in range(len(X_val)): # X_val은 이미 스케일링된 특성들
                val_input_tensor = X_val[i_val_sample].unsqueeze(0).to(device)
                pred_output_scaled_target = model(val_input_tensor) # (1, OUTPUT_SEQ_LENGTH, 1) - 스케일링된 타겟 예측
                
                # 역 스케일링
                dummy_pred_for_inverse = np.zeros((OUTPUT_SEQ_LENGTH, 3))
                dummy_pred_for_inverse[:, 0] = pred_output_scaled_target.cpu().numpy()[0].flatten()
                pred_reconstructed_target_only = scaler.inverse_transform(dummy_pred_for_inverse)[:, 0]

                # 이 예측에 해당하는 원본 타임스탬프 계산 (df_original_for_plotting 기준)
                current_pred_target_start_idx_in_processed = (train_size + i_val_sample) * STRIDE_POINTS + INPUT_SEQ_LENGTH
                current_pred_target_start_idx_in_original = current_pred_target_start_idx_in_processed + rows_dropped_by_ma_std
                current_pred_target_end_idx_in_original = current_pred_target_start_idx_in_original + OUTPUT_SEQ_LENGTH
                
                if current_pred_target_end_idx_in_original <= len(df_original_for_plotting):
                    timestamps_for_pred = df_original_for_plotting['REPORT_DATETIME'].iloc[
                        current_pred_target_start_idx_in_original : current_pred_target_end_idx_in_original
                    ]
                    label_pred = "Model Predictions (Validation Windows)" if not first_prediction_plotted else None
                    if not first_prediction_plotted:
                        sns.lineplot(x=timestamps_for_pred, y=pred_reconstructed_target_only, ax=ax, color='red', alpha=0.6, linewidth=1, label=label_pred)
                        first_prediction_plotted = True
                    else:
                        sns.lineplot(x=timestamps_for_pred, y=pred_reconstructed_target_only, ax=ax, color='red', alpha=0.6, linewidth=1)
                else:
...
# -------------------- 최종 1주일 미래 예측 --------------------
# 스케일링된 전체 특성 데이터셋의 마지막 INPUT_SEQ_LENGTH 만큼을 입력으로 사용
last_sequence_all_features_scaled = series_scaled[-INPUT_SEQ_LENGTH:] # (INPUT_SEQ_LENGTH, 3)
last_sequence_tensor = torch.FloatTensor(last_sequence_all_features_scaled).unsqueeze(0).to(device) # (1, INPUT_SEQ_LENGTH, 3)

model.eval()
with torch.no_grad():
    future_pred_target_scaled = model(last_sequence_tensor) # (1, OUTPUT_SEQ_LENGTH, 1) - 스케일링된 타겟 예측

# 스케일 역변환
dummy_future_pred_for_inverse = np.zeros((OUTPUT_SEQ_LENGTH, 3))
dummy_future_pred_for_inverse[:, 0] = future_pred_target_scaled.cpu().numpy()[0].flatten()
future_prediction_reconstructed_target_only = scaler.inverse_transform(dummy_future_pred_for_inverse)[:, 0]

# # 역 차분을 위한 기준값 (원본 데이터의 가장 마지막 값) -> 차분 안쓰므로 필요 없음
# last_known_actual_value_overall = original_target_series_values[-1]
# # 역 차분 적용 -> 차분 안쓰므로 필요 없음
...
if len(y_val) > 0:
    last_val_input_all_features_scaled_tensor = X_val[-1].unsqueeze(0).to(device) # (1, INPUT_SEQ_LENGTH, 3)

    # 마지막 검증 윈도우에 대한 예측
    with torch.no_grad():
        last_val_pred_target_scaled = model(last_val_input_all_features_scaled_tensor) # (1, OUTPUT_SEQ_LENGTH, 1)
    
    dummy_last_val_pred_for_inverse = np.zeros((OUTPUT_SEQ_LENGTH, 3))
    dummy_last_val_pred_for_inverse[:, 0] = last_val_pred_target_scaled.cpu().numpy()[0].flatten()
    last_val_pred_reconstructed_target_only = scaler.inverse_transform(dummy_last_val_pred_for_inverse)[:, 0]

    # 마지막 검증 윈도우의 타겟 시작 인덱스 (처리된 데이터 기준)
    last_val_target_start_idx_in_processed = (train_size + len(X_val) - 1) * STRIDE_POINTS + INPUT_SEQ_LENGTH
    # 원본 데이터에서의 해당 인덱스
    ts_start_idx_for_last_val_pred_in_original = last_val_target_start_idx_in_processed + rows_dropped_by_ma_std
    ts_end_idx_for_last_val_pred_in_original = ts_start_idx_for_last_val_pred_in_original + OUTPUT_SEQ_LENGTH
    
    if ts_end_idx_for_last_val_pred_in_original <= len(df_original_for_plotting):
        timestamps_for_last_val_target = df_original_for_plotting['REPORT_DATETIME'].iloc[
            ts_start_idx_for_last_val_pred_in_original : ts_end_idx_for_last_val_pred_in_original
        ]
        sns.lineplot(x=timestamps_for_last_val_target, y=last_val_pred_reconstructed_target_only, ax=ax, label=f"Predicted {TARGET_COLUMN} (Last Validation Window)", color='orange', linestyle='--')
    else:
        print("마지막 검증 윈도우의 타임스탬프를 계산할 수 없습니다 (데이터 길이 부족).")

sns.lineplot(x=future_timestamps, y=future_prediction_reconstructed_target_only, ax=ax, label=f"Predicted {TARGET_COLUMN} (Forecast Next {OUTPUT_SEQ_DAYS} Days)", color='red', linewidth=2)
...
print("첫 5개 예측값:", future_prediction_reconstructed_target_only[:5])
print("마지막 5개 예측값:", future_prediction_reconstructed_target_only[-5:])

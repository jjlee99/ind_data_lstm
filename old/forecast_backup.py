...
if not all_val_preds_scaled_list:
    ...
    if all_val_preds_scaled.shape[0] > 0:
        ...
        time_steps_val = range(len(val_actual_sample_inverse))
        sns.lineplot(x=time_steps_val, y=val_actual_sample_inverse.flatten(), ax=ax, label="Actual (Validation First Sample)")
        sns.lineplot(x=time_steps_val, y=val_pred_sample_inverse.flatten(), ax=ax, label="Predicted (Validation First Sample)")
...
if len(X_val) > 0 and len(y_val) > 0:
    ...
    if val_period_actual_end_idx <= len(df):
        ...
        sns.lineplot(x=actual_val_period_timestamps, y=actual_val_period_values, ax=ax, label=f"Actual {TARGET_COLUMN} (Validation Period)", color='blue', linewidth=1.5)
            ...
            for i in range(len(X_val)):
                ...
                if pred_end_idx <= len(df):
                    ...
                    if not first_prediction_plotted:
                        sns.lineplot(x=timestamps_for_pred, y=val_output_actual, ax=ax, color='red', alpha=0.6, linewidth=1, label=label_pred)
                        first_prediction_plotted = True
                    else:
                        sns.lineplot(x=timestamps_for_pred, y=val_output_actual, ax=ax, color='red', alpha=0.6, linewidth=1) # No label for subsequent plots
                else:
...
sns.lineplot(x=df['REPORT_DATETIME'].iloc[-history_to_plot:], y=full_series[-history_to_plot:].flatten(), ax=ax, label=f"Historical {TARGET_COLUMN}", alpha=0.7, linewidth=1)
...
if len(y_val) > 0:
    ...
    if end_idx_target_for_last_val <= len(df):
        ...
        sns.lineplot(x=timestamps_for_last_val_target, y=last_val_pred_actual, ax=ax, label=f"Predicted {TARGET_COLUMN} (Last Validation Window)", color='orange', linestyle='--')
    else:
        print("마지막 검증 윈도우의 타임스탬프를 계산할 수 없습니다 (데이터 길이 부족).")

sns.lineplot(x=future_timestamps, y=future_prediction_actual.flatten(), ax=ax, label=f"Predicted {TARGET_COLUMN} (Forecast Next {OUTPUT_SEQ_DAYS} Days)", color='red', linewidth=2)
